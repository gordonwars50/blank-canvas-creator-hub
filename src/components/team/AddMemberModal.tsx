          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              field
                    ...
                    <FormControl>
                      <Input {...field} placeholder="Enter full name" disabled={isSubmitting} className="border-gray-600 text-white placeholder-gray-400 bg-zinc-900" />
                    </FormControl>
              ...
              field
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input {...field} type="email" placeholder="Enter email address" disabled={isSubmitting} className="border-gray-600 text-white placeholder-gray-400 pl-10 bg-zinc-900" />
                      </div>
                    </FormControl>
                    <FormMessage className="text-red-400" />
                  </FormItem>} />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-3">
                {TEAM_ROLES.map(role => <div key={role.value} className={`p-4 rounded-lg border cursor-pointer transition-colors ${selectedRole === role.value ? 'border-blue-500 bg-blue-500/10' : 'border-gray-600 bg-gray-800/50 hover:bg-gray-800'}`} onClick={() => setSelectedRole(role.value)}>
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${selectedRole === role.value ? 'border-blue-500 bg-blue-500' : 'border-gray-400'}`}>
                          {selectedRole === role.value && <div className="w-full h-full rounded-full bg-white scale-50"></div>}
                        </div>
            ...
            <DialogFooter className="flex gap-3 pt-4">
              <Button type="button" variant="outline" onClick={handleClose} disabled={isSubmitting} className="border-gray-600 text-zinc-800 bg-zinc-50">
                Cancel
              </Button>
