...
const AddMemberModal: React.FC<AddMemberModalProps> = ({
  ...
  const [selectedRole, setSelectedRole] = useState<TeamRole>('Collaborator');
  ...
  return <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="border-gray-700 max-w-md bg-zinc-900">
        <DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-3">
              <Label className="text-gray-300">Default Role</Label>
              <div className="space-y-2">
                {TEAM_ROLES.map(role => <div key={role.value} className={`p-3 rounded-lg border cursor-pointer transition-colors ${selectedRole === role.value ? 'border-blue-500 bg-blue-500/10' : 'border-gray-600 bg-gray-800/50 hover:bg-gray-800'}`} onClick={() => setSelectedRole(role.value)}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`w-4 h-4 rounded-full border-2 ${selectedRole === role.value ? 'border-blue-500 bg-blue-500' : 'border-gray-400'}`}>
